...
%     [Q_01(kk),~,~,~,~,~,~] = Q01(flist(kk).name,path_in);
    ...
    %% QC_06: Check if duration is longer than - is performed after loop - only warnings!
    Q_06(kk) = Q06(flist(kk).name,path_in,duration(kk),filedur,0.01);
    
    %% QC_07: Check if duration is shorter - is performed after loop - only warnings!
    ...
    %% QC_09: Check if file size differs from size of first file - only warnings!
    ...
    %% QC_11: Check for gap between recordings - only warnings!
    if kk==1
