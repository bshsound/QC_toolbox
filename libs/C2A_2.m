...
% new script that is called when files with same filename were found
% new files with same filename will be marked as filename(1).wav oder
% filename(1).wavx etc.
% no files are being overwritten or deleted

function [TL] = C2A_2(sea,station,hydno,source,target,yy,mm,dd)
tic
...
fprintf(fileID,'$totalFiles = $items.Count \n');
fprintf(fileID,'$currentFileIndex = 0 \n');
fprintf(fileID,'foreach ($item in $items) \n');
fprintf(fileID,'{ \n');
fprintf(fileID,'$currentFileIndex++ \n');
fprintf(fileID,'$progressPercent = [math]::Round(($currentFileIndex / $totalFiles) * 100) \n');
fprintf(fileID,'Write-Progress -Activity "Kopiere Dateien" -Status "Datei ${currentFileIndex} von ${totalFiles}: $($item.Name)" -PercentComplete $progressPercent \n');
       fprintf(fileID,'Write-Host $item.Name \n');
            ...
            fprintf(fileID,'$zielDatei = Join-Path -Path $Zielordner -ChildPath $item.Name \n');
            fprintf(fileID,'if (Test-Path -Path $zielDatei) { \n');
            fprintf(fileID,'Write-Host "Datei $($item.Name) existiert bereits im Zielordner. Überspringe..." \n');
            fprintf(fileID,'continue \n');
            fprintf(fileID,'} \n');
            fprintf(fileID,'Write-Host($zielDatei) \n');           
            fprintf(fileID,'Copy-Item -Path $Datei -Destination $zielDatei -Force -Verbose \n');
fprintf(fileID,'}  \n');
...
if station ~= 'HSN'
        ...
        fp = fopen(['X:\Meereskunde\Unterwasserschall\AMSO23\' sea '\' station '\QC\' hydno '\Transferlog\' 'TransferLog_' datestr(start) '.csv'],'w');
end
fprintf(fp, '%s\n','Transfer Logfile');
