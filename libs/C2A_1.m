...
function [TL] = C2A_1(sea,station,hydno,source,target,yy,mm,dd)
tic
...
fprintf(fileID,'{ \n');
       fprintf(fileID,'Write-Host $item.Name \n');
            ...
            fprintf(fileID,'$counter = 1 \n');
            fprintf(fileID,'while (Test-Path -Path $zielDatei) { \n')
            fprintf(fileID,'$filenameWithoutExt = [System.IO.Path]::GetFileNameWithoutExtension($item.Name) \n');
            fprintf(fileID,'$extension = [System.IO.Path]::GetExtension($item.Name) \n');
            fprintf(fileID,'$zielDatei = Join-Path -Path $Zielordner -ChildPath ("{0}({1}){2}" -f $filenameWithoutExt, $counter, $extension) \n');
            fprintf(fileID,'$counter++ \n');
            fprintf(fileID,'} \n');
