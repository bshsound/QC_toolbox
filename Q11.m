...
elseif index == 3
        ...
        delta_t_after = abs(t_after-t);
        delta_t_bef = abs(t-t_bef);
        if delta_t_bef > seconds(duration+duration*limit) || delta_t_after > seconds(duration+duration*limit) || delta_t_bef < seconds(duration-duration*limit) || delta_t_aft < seconds(duration-duration*limit)
            movefile([filepath filename], [filepath 'NoRealData11_' filename]);
            flag = 1;
        else
            flag = 0;
